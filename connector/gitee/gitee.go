...
func (c *Config) Open(id string, logger log.Logger) (connector.Connector, error) {
	...
	giteeConn.httpClient, err = newHTTPClient(c.RootCAs, c.InsecureSkipVerify)
	if err != nil {
		return nil, err
	}

	return giteeConn, err
}
...
func (c *giteeConnector) LoginURL(scopes connector.Scopes, callbackURL, state string) (string, error) {
...
func (c *giteeConnector) HandleCallback(s connector.Scopes, r *http.Request) (identity connector.Identity, err error) {
	q := r.URL.Query()
	...
	if err != nil {
		return identity, fmt.Errorf("gitee connector: failed to get token: %v", err)
	}
	...
	if err != nil {
		return identity, fmt.Errorf("gitee Connector: failed to execute request to userinfo: %v", err)
	}
	defer userInfoResp.Body.Close()

	if userInfoResp.StatusCode != http.StatusOK {
		return identity, fmt.Errorf("gitee Connector: failed to execute request to userinfo: status %d", userInfoResp.StatusCode)
	}
	...
	if err != nil {
		return identity, fmt.Errorf("gitee Connector: failed to parse userinfo: %v", err)
	}

	userID, found := userInfoResult[c.userIDKey].(string)
	if !found {
		return identity, fmt.Errorf("gitee Connector: not found %v claim", c.userIDKey)
	}
	...
	if s.OfflineAccess {
		...
		if err != nil {
			return identity, fmt.Errorf("gitee Connector: failed to parse connector data for offline access: %v", err)
		}
...
func (c *giteeConnector) addGroupsFromMap(groups map[string]struct{}, result map[string]interface{}) error {
...
func (c *giteeConnector) addGroupsFromToken(groups map[string]struct{}, token string) error {
